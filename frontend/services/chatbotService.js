import { askChatbot as askBackendChatbot } from '../../backend/chatbot';

function buildAIConversationalReply(question) {
  ...
  if (['hi', 'hello', 'hey', 'greetings', 'good morning', 'good afternoon', 'good evening', 'who are you', 'what can you do', 'help'].includes(q) || q.startsWith('hi ') || q.startsWith('hello ')) {
...
How can I help you stay safe and prepared today? Feel free to ask any question!`;
  ...
  if (q.includes('what is disaster') || q.includes('define disaster') || q.includes('disaster meaning') || q.includes('what is a disaster') || q.includes('types of disaster')) {
...
Would you like safety tips or an emergency checklist for a specific type of disaster?`;
  ...
  if (q.includes('what is flood') || q.includes('define flood') || q.includes('flood meaning')) {
    return 'A flood is an overflow of water that submerges land that is usually dry. It can happen from heavy rain, river overflow, storm surges, or dam failure, and it can damage homes, roads, and infrastructure.';
  }
...
export async function askChatbot(question) {
  try {
    const backendReply = await askBackendChatbot(question);
  ...
  return buildAIConversationalReply(question);
}
